...
window.JAL.Anchoring = {
  ...
  extractSentenceContext(messageText, quote, startHint) {
    ...
    // Find sentence start: look backwards for sentence-ending punctuation followed by whitespace
    let sentenceStart = 0;
    ...
    // Find sentence end: look forwards from the end of quote for sentence-ending punctuation
  ...
  reanchor(anchor, messageText) {
    ...
    // 5. Final fallback: use startHint position (for equations, special chars, etc.)
    // We return success: true so the highlight is still attempted at the original selection position
    console.log('JAL Anchoring: using startHint fallback at position', startHint);
    return {
      success: true,
      position: startHint,
      confidence: 0.2
  ...
  createRangeForAnchor(anchor, messageElement) {
    ...
    console.log('JAL Anchoring: messageText length:', messageText.length);
    console.log('JAL Anchoring: quoteExact:', anchor.quoteExact.slice(0, 50));

    const result = this.reanchor(anchor, messageText);
    console.log('JAL Anchoring: reanchor result:', result);

    if (!result.success) {
      console.log('JAL Anchoring: reanchor failed');
      return null;
    }

    // Walk the text nodes to find the right position
    const walker = document.createTreeWalker(
      messageElement,
      NodeFilter.SHOW_TEXT
    );
    ...
    // For low-confidence matches (equations, special chars), use a reasonable default length
    // since anchor.quoteExact.length may not match the actual text node length
    let highlightLength = anchor.quoteExact.length;
    if (result.confidence < 0.5) {
      // For low confidence, use a minimum of 10 chars or the quote length, whichever is smaller
      // This prevents highlighting too much or too little
      highlightLength = Math.min(Math.max(highlightLength, 10), 100);
      console.log('JAL Anchoring: low confidence, using adjusted length:', highlightLength);
    }

    const targetEnd = Math.min(result.position + highlightLength, messageText.length);

    console.log('JAL Anchoring: looking for position', targetStart, 'to', targetEnd);
    ...
    console.log('JAL Anchoring: walked', nodeCount, 'text nodes, charCount:', charCount);
    console.log('JAL Anchoring: startNode:', startNode, 'endNode:', endNode);

    // If we found a start but no end, use the last node as end
    if (startNode && !endNode && lastNode) {
      console.log('JAL Anchoring: using last node as end fallback');
      endNode = lastNode;
    ...
    if (!startNode || !endNode) {
      console.log('JAL Anchoring: could not find start/end nodes');
      return null;
    }
    ...
    // If start and end are the same node and start >= end, adjust
    ...
    try {
      ...
      console.log('JAL Anchoring: range created successfully');
      return range;
    } catch (e) {
